...
setwd("/home/cf19810/Dropbox/Ants_behaviour_analysis/Data/")
myrmidon_file <- "R3SP_13-03-21_Capsule_Zones_defined.myrmidon"
##########DEFINE ANTS#############
...
time_start <- fmTimeCPtrFromAnySEXP(tracking_data$getDataInformations()$start + 21*3600)####first time in tracking plus 21 hours, to skip acclimation time
#time_start <- fmTimeCPtrFromAnySEXP(tracking_data$getDataInformations()$end - 24*3600)####last time in tracking minus 24 hours
time_stop  <- fmTimeCPtrFromAnySEXP(tracking_data$getDataInformations()$start + 69*3600) ####pre-tracking period
...
# time_stop <- fmTimeParse("2020-11-05T18:49:21Z")
###above are the times you chose but I recommend the following for the experiment (so it nevers need to be edited):
time_start <- fmTimeCPtrFromAnySEXP(tracking_data$getDataInformations()$start + 69*3600)#### second block, make sure not to overlap with first time block or with exposure time
#time_start <- fmTimeCPtrFromAnySEXP(tracking_data$getDataInformations()$end - 48*3600)####last time in tracking minus 48 hours
time_stop  <- fmTimeCPtrFromAnySEXP(tracking_data$getDataInformations()$start + 69.1*3600) #### made arbitrarily small. pre-tracking period
...
expname <- tracking_data$getDataInformations()[["details"]][["tdd.URI"]]
write.table(positions_summaries, file = paste(expname[1],"_TASK_list_1percent.txt",sep=""))
