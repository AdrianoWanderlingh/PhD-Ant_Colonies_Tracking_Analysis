...
library(datasets)
  ...
  DATADIR <- paste(WORKDIR,"MachineLearning_outcomes_22-08-22",sep="/")
...
#### READ CHOSEN METHOD #######################################################
###############################################################################
chosen <- read.table(paste(WORKDIR,"/MachineLearning_outcomes_22-08-22/quality_scores_CHOSEN.txt",sep=""),header=T,stringsAsFactors = F)
all_methods <- read.table(paste(WORKDIR,"/MachineLearning_outcomes_22-08-22/quality_scores.txt",sep=""),header=T,stringsAsFactors = F)
...
names(rf_model) <- chosen[,"classifier"]
