add_collisions <- function(collisions, trajectory_object,IF_frames,FUZZY_MATCH){
  PLOT_OVERLAP <- FALSE
  ...
  collisions$frames$frame_num <- IF_frames$frame_num  ###NATH_FLAG: why not use the frame_num column in IF_frames?
  ...
  # nrow(trajectory_object)
  # str(trajectory_object)
  ...
  # attributes(trajectory_object$UNIX_time[1])
  # attributes(collisions_merge$UNIX_time[1])
  
  # check that the frame formats are the same
  # trajectory_object$frame[1]
  # collisions_merge$frame     [1]
  
  
  ## check that the pair-time combinations in trajectory_object are in collisions_merge
  # table(  paste(trajectory_object$pair) %in% 
  #           paste(collisions_merge$pair) )
  # table(  paste(trajectory_object$frame) %in% 
  #           paste(collisions_merge$frame) )
  # table(  paste(trajectory_object$pair,trajectory_object$frame) %in% 
  #           paste(collisions_merge$pair,collisions_merge$frame) )
  # table(  paste(trajectory_object$UNIX_secs) %in% 
  #           paste(collisions_merge$UNIX_secs) )
  # table(  paste(trajectory_object$pair,trajectory_object$UNIX_secs) %in% 
  #           paste(collisions_merge$pair,collisions_merge$UNIX_secs) )
  
  ## get a list of the missing pair-frame combinations:
  # paste(trajectory_object$pair,trajectory_object$frame) [!paste(trajectory_object$pair,trajectory_object$frame) %in% paste(collisions_merge$pair,collisions_merge$frame) ]
  
  ## get a list of the missing pair-time combinations:
  # paste(trajectory_object$pair,trajectory_object$UNIX_secs) [!paste(trajectory_object$pair,trajectory_object$UNIX_secs) %in% paste(collisions_merge$pair,collisions_merge$UNIX_secs) ]
  
  ## assign the collision types to trajectory_object, using a loop to subset trajectory_object$pair by each uique pair to ensure we are matching within pairs 
  trajectory_object$types <- NA

  if (PLOT_OVERLAP) {par(mai=c(0.1,0.1,0.17,0), mfrow=c(3,4))}
  for (PR in unique(trajectory_object$pair))
  { print(paste("Adding collision capsule types from collisions_merge to trajectory_object for pair",PR))
    
    ##  sort the entire data frame subset for pair
    trajectory_object_PAIR <- trajectory_object [which(trajectory_object$pair==PR),]
    collisions_merge_PAIR      <- collisions_merge      [which(collisions_merge$pair==PR),]
    
    ## alternative: fuzzy match to one frame
    if (FUZZY_MATCH==TRUE)
    {
      I_in_C_rows1 <- match.closest(x =    trajectory_object_PAIR$frame,
                                    table = collisions_merge_PAIR     $frame, tolerance = 1)
      ## and add back to trajectory_object
      trajectory_object$types [which(trajectory_object$pair==PR)] <- collisions_merge_PAIR$types [I_in_C_rows1]
      ...
      if (PLOT_OVERLAP) 
      {
        ...
        points(x=trajectory_object_PAIR$frame, y=rep(0.5,nrow(trajectory_object_PAIR)), pch="+", col=2) ## man=red
      }
    }
  }##PR
  ...
  #table(trajectory_object$types, useNA="al")
  ...
  # ## copy types across
  # #paste(trajectory_object$pair,trajectory_object$frame) %in% paste(collisions_merge$pair,collisions_merge$frame)
  # 
  ...
  # 
  # trajectory_object <- plyr::join(x = trajectory_object, 
  #                                     y = collisions_merge[,c("frame","pair","types_match")], 
  #                                     by=c("frame","pair"), type="left")
  #}
  
  # #FUNCTION: WHEN pair=pair AND time=UNIX_time -> ASSIGN collisions_merge$types TO trajectory_object
  ...
  # if (FUZZY_MATCH==TRUE)
  # {
  #   I_in_C_rows <- match.closest(x = trajectory_object$UNIX_secs,
  #                                table = collisions_merge$UNIX_secs, tolerance = 0.125)
  #   ## copy tpes across
  #   trajectory_object$types  <- collisions_merge$types [I_in_C_rows]
  # }
  ...
  # for(BH in unique(trajectory_object$BEH))
  # {
  #   for (RW in unique(trajectory_object$ROW))
  #   {
  #     IntPair <- trajectory_object[which(trajectory_object$BEH==BH & trajectory_object$ROW==RW),]
  ...
  # trajectory_object$types_present_absent <- "absent"
  # trajectory_object$types_present_absent[which(!is.na(trajectory_object$types))] <- "present"
  # boxplot(straightline_dist_px ~ types_present_absent, trajectory_object)
