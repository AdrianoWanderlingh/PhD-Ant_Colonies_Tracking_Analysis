...
min(interaction_data$UNIX_time)
max(interaction_data$UNIX_time)

plot(interaction_data$ACT.y ~ interaction_data$ACT.x, asp=1, xlim=c(Xmin,Xmax),ylim=c(Ymin,Ymax))
###############################################################################
###### PARAMETERS PLOTS #######################################################
###############################################################################
...
interaction_data$pair <- apply(interaction_data[,c("ant1","ant2")],1,function(x){paste(sort(x),collapse = "_") })
...
attributes(interaction_data$UNIX_time[1])
...
## check that the pair-time combinations in interaction_data are in collisions_merge
table(  paste(interaction_data$pair) %in% 
        paste(collisions_merge$pair) )
table(  paste(collisions_merge$pair) %in% 
        paste(interaction_data$pair)   )
table(  paste(interaction_data$UNIX_time) %in% 
        paste(collisions_merge$UNIX_time) )
table(  paste(interaction_data$pair,interaction_data$UNIX_time) %in% 
        paste(collisions_merge$pair,collisions_merge$UNIX_time) )
## get a list of the missing pair-time combinations:
paste(interaction_data$pair,interaction_data$UNIX_time) [!paste(interaction_data$pair,interaction_data$UNIX_time) %in% paste(collisions_merge$pair,collisions_merge$UNIX_time) ]

#join dataframes to have collisions on interaction_data  
#FUNCTION: WHEN pair=pair AND time=UNIX_time -> ASSIGN collisions_merge$types TO interaction_data
#or merge as here: 
# ****DOES NOT WORK!!!!****
# new_dataset <- interaction_data %>% dplyr::left_join(collisions_merge, by=c("UNIX_time","pair"))

interaction_data <- plyr::join(x = interaction_data, 
...
par(mfrow=c(3,3), mai=c(0.2,0.2,0.2,0.1))
for(BH in unique(interaction_data$BEH))
  {
  for (RW in unique(interaction_data$ROW))
    {
    IntPair <- interaction_data[which(interaction_data$BEH==BH & interaction_data$ROW==RW),]
    ## check if this interaction is present in collisions
    paste(IntPair$pair[1],IntPair)
    ## plot it
    plot(NA, xlim=range(c(IntPair$ACT.x,IntPair$REC.x),na.rm=T), 
             ylim=range(c(IntPair$ACT.y,IntPair$REC.y),na.rm=T), type="n", main=paste(BH,RW))
...
interaction_data$types_present_absent <- "absent"
interaction_data$types_present_absent[which(!is.na(interaction_data$types))] <- "present"
boxplot(straightline_dist_px ~ types_present_absent, interaction_data)
