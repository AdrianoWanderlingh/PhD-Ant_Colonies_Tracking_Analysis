...
SpaceUse <- function(exp, start, end){
  ...
  # hour_chunk_start <-  sort(seq(3, 51, by = 3),decreasing = TRUE ) #3h bouts chunks 
  # hour_chunk_start <- hour_chunk_start[hour_chunk_start!= 27]
  # TimeWindow <-  3
  # 
  # 
  #define zones
  ...
  #   loop_N <- loop_N + 1
  #   print(paste("Computing chunk",loop_N,"of",length(hour_chunk_start), "|| Time interval from hour",HOUR_start,"to",(HOUR_start-TimeWindow),"until exp end",sep = " "))
    ...
    # colnames(positions_summaries_LOOP) [match("nb_frames_outside",colnames(positions_summaries_LOOP))] <-  if (HOUR_start>24) { #pre
    #                                                                                                           paste0("nb_frames_outside",loop_N,"pre")
    #                                                                                                         }else{#post
    #                                                                                                           paste0("nb_frames_outside",loop_N,"post")
    #                                                                                                         }
    # 
    # colnames(positions_summaries_LOOP) [match("nb_frames_inside",colnames(positions_summaries_LOOP))] <-  if (HOUR_start>24) { #pre
    #                                                                                                           paste0("nb_frames_inside",loop_N,"pre")
    #                                                                                                         }else{#post
    #                                                                                                           paste0("nb_frames_inside",loop_N,"post")
    #                                                                                                         }
    # 
    # 
  ...
  SpaceUsage <- positions_summaries
  
  ##############################################################
  ## OUTPUT SpaceUse!!
  ## OUTPUT WITH THE SAME STRUCTURE OF EXP 1 OUTPUT
  #############################################################
  ...
  # SpaceUsage[which(is.na(SpaceUsage$AntTask)),"AntTask"] <- "nurse"
  

  SpaceUsage <- SpaceUsage[order(SpaceUsage$antID),]
  
  rm(list=ls()[which(!ls()%in%c("positions_summaries_list","positions_SUMS","exp","foraging_zone","AntID_list","SpaceUsage"))]) #close experiment
  gc()
  mallinfo::malloc.trim(0L)
