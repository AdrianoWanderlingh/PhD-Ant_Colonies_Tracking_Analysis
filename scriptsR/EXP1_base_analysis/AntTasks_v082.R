...
AntTasks <- function(exp){
  ...
  for (HOUR_start in hour_chunk_start) {
    ...
    stop <- fmQueryGetDataInformations(exp)$end - (HOUR_start-12)*3600
    ...
    rm(list=ls()[which(!ls()%in%c("positions_summaries_list","exp","foraging_zone","nest_zone","AntID_list","hour_chunk_start","loop_N"))]) #close experiment
    gc()
