...
INTERACTIONS <- file.path(DATADIR, paste0("Exp1_Results_", Sys.Date()), "INTERACTIONS_by_period.txt") # (saved INSIDE the folder)
...
for (REP.n in 1:length(files_list)) {
  ...
  for (REP.FILES in REP.filefolder) {
    ...
    Interactions_PERIOD <- data.frame()
    ...
    for (PERIOD in c("pre","post")) {
      ...
      Interactions <- compute_Interactions(e = e, start = time_start, end = time_stop, gap = MAX_INTERACTION_GAP)
      
      ### Individual
      # Add metadata info
      Interactions_PERIOD <- cbind(data.frame(
        randy = REP.FILES, REP_treat = REP_TREAT, period = PERIOD,Interactions,
        stringsAsFactors = F
      ))
      # stack
      Interactions_total <- rbind(Interactions_total, Interactions_PERIOD)
    }
    ...
    # # COMPUTE NETWORK
    # warning("change to:  compute_G(e, Interactions) ")
    # Graph <- compute_G(e = e, start = time_start, end = time_stop, gap = MAX_INTERACTION_GAP)
    # # COMPUTE NETWORK PROPERTIES (collective and individual)
    # Network_summ_prop_hour <- NetProperties(graph = Graph)
    # 
    ...
    if (file.exists(INTERACTIONS)) {
      write.table(Interactions_total, file = INTERACTIONS, append = T, col.names = F, row.names = F, quote = T, sep = ",")
    } else {
      write.table(Interactions_total, file = INTERACTIONS, append = F, col.names = T, row.names = F, quote = T, sep = ",")
    }
