...
setwd("/media/adriano/DISK3/ADRIANO/NURSEID/R10SP_14-05-21.0000_nurseid_folder/")
myrmidon_file <- "R10SP_14-05-21.0000_nurseid.myrmidon"
##########DEFINE ANTS#############
...
time_start <- fmTimeCPtrFromAnySEXP(tracking_data$getDataInformations()$end - 12*3600)####last time in tracking minus 48 hours
...
time_stop  <- fmTimeCPtrFromAnySEXP(tracking_data$getDataInformations()$end - 12*3600) ####last time in tracking
###QUERY 3: fmQueryComputeAntTrajectories()
positions                 <- fmQueryComputeAntTrajectories(tracking_data,start = time_start,end = time_stop,maximumGap = fmHour(24*365),computeZones = TRUE)
positions_summaries       <- positions$trajectory_summary
positions_summaries       <- data.frame(index=1:nrow(positions_summaries),positions_summaries,stringsAsFactors = F)
...
write.table(nurse_list, file = paste(tracking_data$getDataInformations()[["details"]][["tdd.URI"]],"_nurses_list_1percent24h.txt",sep=""), append = FALSE,
